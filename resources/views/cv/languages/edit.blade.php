<x-app-layout>
<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar idioma</h2>
</x-slot>

<div class="py-6 max-w-2xl mx-auto p-6">
    <p class="text-sm text-gray-500 uppercase tracking-wide mb-1">Paso 6 - Idiomas</p>
    <h1 class="text-2xl font-semibold mb-2">Editar idioma</h1>
    <form method="POST" action="{{ route('cv.languages.update', $language) }}" class="space-y-4">
        @csrf
        @method('PUT')
        @include('cv.languages.form', ['lang' => $language])
