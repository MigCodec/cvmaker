<x-app-layout>
<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nueva habilidad</h2>
</x-slot>

<div class="py-6 max-w-2xl mx-auto p-6">
    <p class="text-sm text-gray-500 uppercase tracking-wide mb-1">Paso 5 - Habilidades</p>
    <h1 class="text-2xl font-semibold mb-2">Nueva habilidad</h1>
    <p class="text-sm text-gray-600 mb-4">Captura cada skill por separado para poder ordenarlas y sumarlas rapido.</p>
    <form method="POST" action="{{ route('cv.skills.store') }}" class="space-y-4">
        @csrf
        @include('cv.skills.form')
